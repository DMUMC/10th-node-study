...
export const listStoreMissions = async (storeId, cursor) => {
  const storeExists = await existsStore(storeId)
  if (!storeExists) throw new StoreNotFoundError(storeId)
  ...
  return {
    ...
    nextCursor: missions.length === 10 ? missions[missions.length - 1].id : null,
  }
}
